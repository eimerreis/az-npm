...
## User Stories
- As a developer with Azure CLI signed in, I want to run `az-npm` in my project and have my `.npmrc` or `bunfig.toml` credentials updated automatically so I can install private packages without manual token management.
- As a CI pipeline operator, I want `az-npm` to pick up `SYSTEM_ACCESSTOKEN` or similar env vars and authenticate non-interactively so builds work without interactive Azure CLI login.
...
| --- | --- |
| `src/lib/detect.ts` | Detect a single package manager from lockfiles and fail when multiple package-manager indicators are present |
| `src/lib/feed.ts` | Parse Azure DevOps registry URLs from Bun project config or project `.npmrc`; support both `pkgs.dev.azure.com` and `*.pkgs.visualstudio.com` patterns |
...
- Discover registry settings from project-level config first (`bunfig.toml` for bun when applicable, `.npmrc` fallback for Bun, `.npmrc` for npm/pnpm)
...
- Preserve the current `_authToken` contract for `.npmrc` auth entries
...
- [ ] Task 3: `feed.ts` — parse Azure DevOps registry URLs from package-manager-aware project config and extract structured feed information with unit tests.
...
- [ ] Detects npm, pnpm, and bun from their respective lockfiles, including legacy `bun.lockb`, and fails clearly when multiple package-manager indicators are present.
...
- [ ] `.npmrc` auth entries are written correctly using the current `_authToken` model.
- [ ] `bunfig.toml` auth entries are written correctly for Bun registries.
- [ ] Bun auth fails clearly when the discovered Azure feed is unscoped.
...
- [ ] Clear errors are returned when no lockfile is found, multiple package-manager indicators are present, no Azure DevOps registry is configured, Azure CLI is not authenticated, or token acquisition fails.
